...
public class XML {
     ...
     */
    private void append(String element, String text) {
        Element newElement = ownerDoc.createElement(element);
     ...
     * @param map
     *         a map of entries containing element name as the key and
     *         text value as the key value.
     */
    private void append(Map<String, String> map) {
    ...
    private Optional<Element> getElementValue(List<String> list, Element nextElement) {
            ...
            if (!nextElementName.contains("~")){
                try {
                    ...
                    if (n2nString != null && n2nString.contains("~")) {
                        ...
                        return Optional.of((Element)nextElementList.item(0));
                    }
        ...
        return (nextElement == null) ? Optional.empty() : Optional.of(nextElement);
    }
    ...
    public String findValue(List<String> list) {
        ...
        Optional<Element> optionalElement = getElementValue(list, nextElement);
        if (!optionalElement.isPresent()) {
            return null;
        } else {
            nextElement = optionalElement.get();
        }
