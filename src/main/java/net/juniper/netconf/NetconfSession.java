...
public class NetconfSession {
    ...
    protected NetconfSession(Session netconfSession, String hello, 
        ...
        InputStream stdout = netconfSession.getInputStream();
    ...
    private XML convertToXML(String xml) throws SAXException, IOException {
        if (xml.contains("netconf error: syntax error")) {
            throw new NetconfException(String.format("Netconf server detected an error:%n %s", xml));
        }
    ...
    private void sendHello(String hello) throws IOException {
        log.debug("Sending Netconf Capabilities Hello message. {}", hello);
    ...
    private String getRpcReply(String rpc) throws IOException {
        netconfSession.getOutputStream().write(rpc.getBytes(Charset.defaultCharset()));
        netconfSession.getOutputStream().flush();
     ...
     *
     * @param rpcContent an RPC command that may or may not be wrapped in  &lt or &gt
     * @return a string of the RPC command wrapped in &lt;rpc&gt;&lt &gt;&lt;/rpc&gt
     * @throws IllegalArgumentException if null is passed in as the rpcContent.
     */
    private String fixupRpc(String rpcContent) throws IllegalArgumentException {
        if (rpcContent == null) {
